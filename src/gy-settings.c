...
#define GET_PRIVATE(instance) G_TYPE_INSTANCE_GET_PRIVATE (instance, GY_TYPE_SETTINGS, GySettingsPrivate)
...
gy_settings_class_init (GySettingsClass *klass)
{
  ...
  signals[FONTS_CHANGED] = g_signal_new ("fonts-changed",
    ...
    signals[PATHS_CHANGED] = g_signal_new ("paths-changed",
                                           G_TYPE_FROM_CLASS (klass),
                                           G_SIGNAL_RUN_FIRST,
...
gy_settings_get_path_dictionary (GySettings  *settings,
                                 const gchar *key)
{
  return g_settings_get_string (settings->priv->settings_dictionary, key);
}
...
gy_settings_get_use_paths_system (GySettings  *settings,
                                  const gchar *key)
{
  return g_settings_get_boolean (settings->priv->settings_dictionary, key);
}
