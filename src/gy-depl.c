...
end_element_cb (ParserContext *context)
{
    ...
    {
	g_warning ("An excess tag </%s>. "
		   "The tag will be skipped. A formatting is continued.",
	...
	if ((prev = last->prev))
	{
	    ...
	    {
		g_warning ("The tags </%s> and </%s> have a wrongly position (ones are reversed). "
			   "A formatting is continued.",
	...
	{
	    context->is_formating_error = 1;
	    g_critical ("An undefined error! A formatting is broken. "
	    		"A text will be inserted, but it will not be formatted.");
	    return;
	}
    }
